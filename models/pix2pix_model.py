...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['real_A', 'fake_B', 'real_B', 'dmask']
        ...
        self.detector = GenMask(opt)   # yolo3 detector
    ...
    def set_input(self, input):
        ...
        self.dmask, self.im = self.detector.get_suitablemask(input)  # 1,1,256,256
    ...
    def backward_D(self):
        ...
        if Dxy:
            ...
            # Fake; stop backprop to the generator by detaching fake_B
            fake_AB = torch.cat((self.real_A, self.fake_B), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
            pred_fake = self.netD(fake_AB.detach())*torch.tensor(1 + gamma*np.array(self.im.resize((30,30))), dtype=torch.float32).to(self.device).expand(1,1,30,30)
            self.loss_D_fake = self.criterionGAN(pred_fake, False)
            # Real
            real_AB = torch.cat((self.real_A, self.real_B), 1)
            pred_real = self.netD(real_AB)*torch.tensor(1 + gamma*np.array(self.im.resize((30,30))), dtype=torch.float32).to(self.device).expand(1,1,30,30)
            self.loss_D_real = self.criterionGAN(pred_real, True)
            # combine loss and calculate gradients
            self.loss_D = (self.loss_D_fake + self.loss_D_real) * 0.5
            self.loss_D.backward()
